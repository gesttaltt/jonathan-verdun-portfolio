  ...
  return (
    <header className="animate-hero mb-16 flex flex-col items-start gap-8 md:mb-24 md:flex-row md:items-end md:justify-between">
      <div>
        ...
        <h1 className="light:from-zinc-900 light:via-zinc-800 light:to-zinc-600 mb-6 bg-gradient-to-r from-white via-zinc-200 to-zinc-500 bg-clip-text text-4xl font-extrabold tracking-tighter text-transparent sm:text-5xl md:text-6xl lg:text-8xl 2xl:text-9xl dark:from-white dark:via-zinc-200 dark:to-zinc-500">
          {siteConfig.name}
        </h1>
        <div className="flex flex-wrap items-center gap-4 text-sm font-medium sm:gap-6">
          <Link
            ...
            className="group focus-visible:ring-offset-background light:hover:text-zinc-900 flex min-h-[44px] items-center gap-2 rounded text-zinc-300 transition-colors hover:text-white focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 focus-visible:outline-none dark:hover:text-white"
          >
      ...
      {siteConfig.workHistory.length > 0 && (
        <div className="light:border-zinc-200 light:bg-zinc-100/50 flex flex-col gap-3 rounded-2xl border border-white/8 bg-white/8 p-5 backdrop-blur-md dark:border-white/8 dark:bg-white/8">
          <div className="flex items-center gap-3 text-xs">
            <span className="inline-flex h-2.5 w-2.5 rounded-full bg-green-500"></span>
            <span className="light:text-zinc-500 font-bold tracking-widest text-zinc-300 uppercase dark:text-zinc-300">
              {t.workHistoryLabel}
            </span>
          </div>
          {siteConfig.workHistory.map((entry) => (
            <div key={entry.organization} className="flex flex-col gap-0.5">
              ...
              {(t.workHistoryRoles[entry.organization] ||
                ...
                entry.period) && (
                <p
                  className="light:text-zinc-500 text-xs text-zinc-300 dark:text-zinc-300"
              ...
              {t.workHistoryDescriptions[entry.organization] && (
                <p className="light:text-zinc-500 text-xs text-zinc-300 dark:text-zinc-300">
                  {t.workHistoryDescriptions[entry.organization]}
                </p>
              )}
            </div>
