...
export const BioinformaticsResearchCard: React.FC<BioinformaticsResearchCardProps> = ({ spec }) => {
  ...
  return (
    <m.div
      ...
      className="group relative overflow-hidden rounded-2xl border border-white/10 bg-white/5 p-6 transition-all duration-300 hover:border-purple-500/30 hover:bg-white/10 hover:shadow-[0_0_24px_var(--glow-purple)]"
    >
      <div className="mb-4 flex items-center gap-2">
        <h3 className="flex items-center gap-2 text-base font-bold tracking-wide text-purple-200 uppercase">
          <Dna className="h-4 w-4" /> {label}
        </h3>
      </div>
      <p className="mb-4 text-xs leading-relaxed text-zinc-300">{description}</p>
      <div className="space-y-3 text-sm text-zinc-300">
        ...
        {spec.link && (
          <Link
            ...
            className="focus-visible:ring-offset-background inline-flex items-center gap-1 rounded text-[10px] text-zinc-300 transition-colors hover:text-purple-300 focus-visible:ring-2 focus-visible:ring-purple-500 focus-visible:ring-offset-2 focus-visible:outline-none sm:text-xs"
          >
