// ─── Vertex Shader ────────────────────────────────────────────────────────────
// Displaces vertices with Simplex noise, detects hover proximity in
// screen space, and sizes each point sprite based on displacement + pulse + hover.
// Supports per-vertex colors for fractal tree visualization.
export const vertexShader = `
    ...
    void main() {
        ...
        vec3 ambientPos = position + vec3(0.0, 0.0, noise * 0.5);
        ...
        // Ultrametric Shift: branches reorganize based on cursor proximity.
        // Higher hover values shift the Z-axis more dramatically, simulating 
        // a p-adic distance contraction.
        vec3 shiftedPos = ambientPos;
        shiftedPos.z += vHover * sin(ambientPos.x * 2.0 + ambientPos.y * 2.0 + effectiveTime) * 1.5;
        
        vec3 finalPos = shiftedPos + vec3(0.0, 0.0, vHover * 0.8);
        mvPosition = modelViewMatrix * vec4(finalPos, 1.0);
        gl_Position = projectionMatrix * mvPosition;

        gl_PointSize = (4.0 + noise * 2.0 + vPulse * 8.0 + vHover * 6.0) * (1.0 / -mvPosition.z);
    }
`
